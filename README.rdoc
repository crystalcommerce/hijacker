...
One application, multiple client databases. Although customizable, by default uses a combination of database and regular expression matching againts the host domain to figure out which database to connect to.
...
For copy/pasters, a shorter version:
    ...
    hijack_connection({
      :static_routes => Proc.new { "site_#{env}" if !Rails.env == "production" },
